...
var suppressAccessibleOutput = false;  // Should we suppress accessible output?
var commandHistory = [];
var historyIndex = -1;
var currentCommand = ''; // Store what user is currently typing
...
{   
    ...
    // Store original text for accessible output (before cursor is added)
    var accessibleText = aText;
    ...
    if (sharpKpos!=-1)
    {
        ...
        accessibleText = aText; // Update accessible text too
        inANYKEY = inMORE = true;
        writeTextDone = done;
        if (flags.getFlag(FTIMEOUT))
        if (flags.getFlag(FTIMEOUT_CONTROL) & 0x02)
    ...
    // Send to accessible output ONCE, before rendering
    if (accessibleText.length > 0 && !accessibleText.endsWith('_')) {
        writeTextAccessible(accessibleText);
    }
    
    // 4.- Print what it should be printed now (existing code)
    var aWord = '';
...
function writeTextAccessible(text) {
    if (suppressAccessibleOutput) return;
    
    const output = document.getElementById('accessible-output');
    if (output) {
        // Remove special characters and format for screen readers
        let cleanText = text
            .replace(/[\x00-\x0B\x0E-\x1F]/g, '\n') // Convert control chars to newlines except CR
            .replace(/\r/g, '\n') // Convert CR to newline
            .replace(/_/g, '') // Remove cursor characters
            .replace(/\n\n+/g, '\n') // Remove multiple newlines
            .trim();
        
        if (cleanText.length > 0) {
            // Don't duplicate if this exact text was just added
            const lastText = output.textContent.slice(-cleanText.length);
            if (lastText !== cleanText) {
                const textNode = document.createTextNode(cleanText + '\n');
                output.appendChild(textNode);
                
                // Keep output manageable - limit to last 500 lines
...
// The original PCDAAD readText is split in parts as the key pressing part should be somewhere in an eveny handler
function readText(thePrompt)
{
    ...
    flags.setFlag(FTIMEOUT_CONTROL, flags.getFlag(FTIMEOUT_CONTROL)& 0x3F);
    
    // Don't send the prompt with cursor to accessible output
    suppressAccessibleOutput = true;
    writeText(readTextStr + '_');
    suppressAccessibleOutput = false;
    
    // Setup accessible input
    const accessibleInput = document.getElementById('accessible-input');
    if (accessibleInput) {
        accessibleInput.value = '';
        accessibleInput.placeholder = thePrompt;
        ...
        // Add new listener with history support
        accessibleInput.onkeydown = function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                const command = accessibleInput.value.trim();
                ...
                if (command.length > 0) {
                    ...
                    inputBuffer = command;
                    // Echo the command to accessible output WITH the actual command text
                    writeTextAccessible('> ' + command);
                    accessibleInput.value = '';
                ...
                if (commandHistory.length > 0) {
                    ...
                    if (historyIndex === commandHistory.length) {
                        currentCommand = accessibleInput.value;
                    }
                    
                    if (historyIndex > 0) {
                        historyIndex--;
                        accessibleInput.value = commandHistory[historyIndex];
                        // Move cursor to end
                        setTimeout(() => {
                            accessibleInput.selectionStart = accessibleInput.selectionEnd = accessibleInput.value.length;
                        }, 0);
                    }
                }
            }
                ...
                if (commandHistory.length > 0 && historyIndex < commandHistory.length) {
                    ...
                    if (historyIndex === commandHistory.length) {
                        ...
                        accessibleInput.value = currentCommand;
                    } else {
                        accessibleInput.value = commandHistory[historyIndex];
                    }
                    // Move cursor to end
...
function readTextB(key)
{
    ...
    // Suppress accessible output for cursor updates
    suppressAccessibleOutput = true;
    ...
    if (keyCode==13)
    {
        ...
        if (inputBuffer.length)
        {
            ...
            // Echo to accessible output WITH the actual command
            writeTextAccessible('> ' + inputBuffer);
...
//    writeTextAccessible(getMessage(DDB.header.sysmessPos, sysno));
}
...
function _ANYKEY() 
{
    ...
    inANYKEY = true;
        writeTextAccessible(getMessage(DDB.header.sysmessPos, SM16)); // Output the press any key message to the accessible output
